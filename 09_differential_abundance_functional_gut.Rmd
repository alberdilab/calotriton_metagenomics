...
adonis2(GIFTs_functions_community_dist ~ environment,
        by="terms", 
        data = sample_metadata %>% arrange(match(sample,labels(GIFTs_functions_community_dist))), 
        permutations = 999)
```
...
GIFTs_functions_community_dist %>%
  ...
  ungroup() %>%
  ggplot(aes(x = NMDS1, y = NMDS2, color = environment, shape=river)) +
  geom_point(size = 3) +
...
#  geom_point(size=4) + 
  ...
  labs(fill="Elevation")
```
```{r plot3_gut, comment="", message=FALSE, warning=FALSE, fig.height=8, fig.width=14, fig.fullwidth=TRUE}
...
```{r  domain_sig_gut, comment="", message=FALSE, warning=FALSE}
...
```{r domain_plot_gut, comment="", message=FALSE, warning=FALSE}
