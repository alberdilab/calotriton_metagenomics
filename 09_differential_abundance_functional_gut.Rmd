...
element_gift <- GIFTs_elements_community %>% 
  ...
  left_join(., sample_metadata[c(1,13)], by=join_by("sample"=="sample"))
```
...
#  geom_point(size=4) + 
  ...
  labs(fill="Elevation")
```
```{r plot3, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=14, fig.fullwidth=TRUE}
...
function_gift <- GIFTs_functions_community %>% 
  as.data.frame() %>% 
  rownames_to_column(., "sample") %>% 
  merge(., sample_metadata[c(1,13)], by="sample")
```
