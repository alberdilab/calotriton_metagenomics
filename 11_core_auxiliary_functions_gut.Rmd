...
```{r functional_difference_source_load_gut, comment="", message=FALSE, warning=FALSE}
...
```{r microbiota_fractioning_gut, comment="", message=FALSE, warning=FALSE}
core_microbiota %>% 
  group_by(type_river) %>% 
  summarise(count=n())
```

```{r core_microbiota_selection_microdiversity, comment="", message=FALSE, warning=FALSE}
...
```{r high_microbiota_selection_microdiversity, comment="", message=FALSE, warning=FALSE}
environment_prevalence %>%
  group_by(genome) %>%
...
```{r low_microbiota_selection_microdiversity, comment="", message=FALSE, warning=FALSE}
environment_prevalence %>%
  group_by(genome) %>%
...
```{r microbiota_fractioning_plot_gut ,comment="", message=FALSE, warning=FALSE}
...
```{r core_microbiota_genome_size_gut ,comment="", message=FALSE, warning=FALSE}
core_microbiota %>% 
...
```{r functional_partitioning_mci_test_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
...
```{r functional_partitioning_mci_test_gut2, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r functional_partitioning_domain_test_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r functional_partitioning_domain_plot_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r functional_partitioning_function_test_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r functional_partitioning_function_plot_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r endemics_mci_plot_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r endemics_mci_test_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r endemics_function_test_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r functional_function_plot_gut, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r chart1_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_endemic_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_test_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_plot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>%
...
```
```{r gift_core_mci_plot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
...
```{r gift_core_mci_boxplot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_core_mci_test_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_core_test_plot_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_permanova_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_pcoa_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_core_ordination_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    pivot_wider(names_from="gift",values_from="value") %>% 
    column_to_rownames(var="sample") %>%
...
```{r gift_endemic_plot_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_endemic_mci_plot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_endemic_mci_boxplot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_endemic_mci_test_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_endemic_test_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_endemic_test_plot_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_endemic_permanova_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_endemic_pcoa_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_endemic_ordination_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    pivot_wider(names_from="gift",values_from="value") %>% 
    column_to_rownames(var="sample") %>%
...
```{r gift_marginal_plot_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_marginal_mci_plot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_marginal_mci_boxplot_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
    ...
    ggplot(aes(x=environment,y=mci, group=environment)) +
        ...
        theme_minimal()
```
```{r gift_marginal_mci_test_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_marginal_test_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_marginal_test_plot_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_marginal_permanova_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_marginal_pcoa_gut, comment="", message=FALSE, warning=FALSE}
...
```{r gift_marginal_ordination_gut, comment="", message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    filter(!is.na(value)) %>% 
    pivot_wider(names_from="gift",values_from="value") %>% 
