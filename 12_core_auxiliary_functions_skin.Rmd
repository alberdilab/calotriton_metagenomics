...
```{r functional_difference_source_load,comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r microbiota_fractioning,comment="", echo=FALSE, message=FALSE, warning=FALSE}
core_microbiota %>% 
  group_by(type_river) %>% 
  summarise(count=n())
```
...
```{r microbiota_fractioning_plot ,comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r relative_abundance_distribution, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_counts_filt %>%
...
```{r core_microbiota_genome_size_skin ,comment="", echo=FALSE, message=FALSE, warning=FALSE}
core_microbiota %>% 
...
```{r functional_partitioning_mci_test_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
...
```{r functional_partitioning_mci_test_skin2, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r functional_partitioning_function_test_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r functional_partitioning_function_plot_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r endemics_mci_plot_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r endemics_mci_test_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    to.domains(., GIFT_db) %>%
...
```{r endemics_function_test_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r functional_function_plot_skin, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_gifts %>%
    to.elements(., GIFT_db) %>%
    to.functions(., GIFT_db) %>%
    as.data.frame() %>%
...
```{r core_taxonomy_barplot, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate_at(vars(-genome),~./sum(.)) %>% #apply TSS nornalisation
  pivot_longer(-genome, names_to = "sample", values_to = "count") %>% #reduce to minimum number of columns
  left_join(., genome_metadata, by = join_by(genome == genome)) %>% #append genome metadata
...
```{r endemic_taxonomy_barplot, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate_at(vars(-genome),~./sum(.)) %>% #apply TSS nornalisation
  pivot_longer(-genome, names_to = "sample", values_to = "count") %>% #reduce to minimum number of columns
  left_join(., genome_metadata, by = join_by(genome == genome)) %>% #append genome metadata
...
```{r marginal_taxonomy_barplot, fig.height=6, fig.width=10, fig.fullwidth=TRUE}
genome_counts_filt %>%
  mutate_at(vars(-genome),~./sum(.)) %>% #apply TSS nornalisation
  pivot_longer(-genome, names_to = "sample", values_to = "count") %>% #reduce to minimum number of columns
  left_join(., genome_metadata, by = join_by(genome == genome)) %>% #append genome metadata
...
```{r pcoa_ordination_plot_enrichment, message=FALSE, fig.height=8, fig.width=10, fig.fullwidth=TRUE}
...
```{r pcoa_ordination_plot_fractioning, message=FALSE, fig.height=8, fig.width=10, fig.fullwidth=TRUE}
...
```{r gift_core_endemic, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
core_genomes <- core_microbiota %>% filter(type=="core") %>% filter(genome %in% rownames(genome_gifts)) %>%  pull(genome)
endemic_genomes <- core_microbiota %>% filter(type=="endemic") %>% filter(genome %in% rownames(genome_gifts)) %>% pull(genome)
marginal_genomes <- core_microbiota %>% filter(type=="marginal") %>% filter(genome %in% rownames(genome_gifts)) %>% pull(genome)
...
```{r gift_core_test, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_core_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>%
...
```{r gift_core_mci_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
...
```{r gift_ecore_mci_boxplot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_core_test_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_core_permanova, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_core_pcoa, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_core_ordination, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_core %>%
    filter(sample!="EHI02625") %>% 
    pivot_wider(names_from="gift",values_from="value") %>% 
    column_to_rownames(var="sample") %>%
...
```{r gift_endemic_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r giftendemic_mci_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_endemic_mci_boxplot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_endemic_test, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_endemic_test_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_endemic_permanova, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_endemic_pcoa, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_endemic_ordination, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_endemic %>%
    filter(sample!="EHI02625") %>% 
    pivot_wider(names_from="gift",values_from="value") %>% 
    column_to_rownames(var="sample") %>%
...
```{r gift_marginal_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_marginal_mci_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_marginal_mci_boxplot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    group_by(sample) %>% 
    summarise(mci=mean(value,na.rm=TRUE)) %>% 
...
```{r gift_marginal_test, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_marginal_test_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE}
GIFTs_community_marginal %>%
    filter(sample!="EHI02625") %>% 
    left_join(sample_metadata, by = "sample") %>% 
    group_by(gift) %>%
...
```{r gift_marginal_permanova, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_marginal_pcoa, comment="", echo=FALSE, message=FALSE, warning=FALSE}
...
```{r gift_marginal_ordination, comment="", echo=FALSE, message=FALSE, warning=FALSE}
