...
colNames <- names(element_gift_names)[2:29]
for(i in colNames){
...
```
```{r skin_commun_wilcox_elem_plot, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=12, fig.fullwidth=TRUE, eval=FALSE}
difference_table %>%
...
#  geom_point(size=4) + 
  ...
  labs(fill="Elevation")
```
```{r skin_plot3, comment="", echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=14, fig.fullwidth=TRUE}
...
colNames <- names(function_gift_names)[2:4]
for(i in colNames){
