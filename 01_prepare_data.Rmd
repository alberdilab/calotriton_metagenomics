...
sample_metadata <- read_csv("data/PR_Preprocessing-Full view.csv") %>%
  select("EHI_number","capture_id_flat", "species","sample_type", "sex", "latitude", "longitude","singlem_fraction","reads_post_fastp","metagenomic_bases","host_bases", "bases_lost_fastp_percent_flat") %>%
    rename(sample=1) %>%
...
genome_gifts <- genome_gifts_raw[genome_counts_filt$genome,]
...
sample_metadata <- read_csv("data/PR_Preprocessing-Full view.csv") %>%
  ...
  select(-species, -sample_type, -latitude, -longitude)
```
