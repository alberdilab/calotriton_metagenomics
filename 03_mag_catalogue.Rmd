...
```{r gut_genome_taxonomy_bacteria}
genome_metadata %>% 
  group_by(phylum) %>%
  summarise(mag_n=n()) %>%
  arrange(-mag_n) %>%
...
```{r gut_function_heatmap, message=FALSE, warning=FALSE, fig.height=10, fig.width=10, fig.fullwidth=TRUE}
...
```{r gut_function_ordination, message=FALSE, fig.height=8, fig.width=10, fig.fullwidth=TRUE}
...
function_table <- bind_rows(function_table_gut,function_table_skin)

tSNE_function <- function_table %>% 
   Rtsne(X=., dims = 2, check_duplicates = FALSE)

genome_metadata <- bind_rows(genome_metadata_gut %>% mutate(type="gut"),genome_metadata_skin %>% mutate(type="skin"))
...
tSNE_function$Y %>%
                ...
                inner_join(genome_metadata, by="genome") %>%
                rename(tSNE1="V1", tSNE2="V2") %>%
                select(genome,phylum,tSNE1,tSNE2, length, type) %>%
                ggplot(aes(x = tSNE1, y = tSNE2, color = phylum, size=length, shape=type))+
                            geom_point(alpha=0.7) +
                            scale_color_manual(values=phylum_colors) +
                            theme_minimal() +
                labs(color="Phylum", size="Genome size") +
