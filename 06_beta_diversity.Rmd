...
sample_metadata$location<- paste(sample_metadata$region,sample_metadata$environment, sep="_")
#sample_metadata$all<- paste(sample_metadata$location,sample_metadata$treatment, sep="_")
```
...
beta_q0n$S %>%
  ...
  ungroup() %>%
  ggplot(aes(x = NMDS1, y = NMDS2, color = location, fill = location, shape = as.factor(treatment))) +
  geom_point(size = 4) +
